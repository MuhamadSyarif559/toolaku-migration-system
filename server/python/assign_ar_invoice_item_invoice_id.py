...
def parse_args():
    parser = argparse.ArgumentParser(description="Match InvoiceID onto AR item target rows.")
...
def load_customer_invoice_map(customer_target_file):
    ...
    for row in sheet.iter_rows(min_row=2, values_only=True):
        ...
        if doc_code not in (None, ""):
            invoice_by_doccode[str(doc_code).strip()] = invoice_id
    return invoice_by_doccode


def build_qne_item_queues(qne_file, invoice_by_doccode):
    ...
    for row_number, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):
        ...
        invoice_id = invoice_by_doccode.get(doc_code_text)
...
def main():
    args = parse_args()
    invoice_by_doccode = load_customer_invoice_map(Path(args.customer_target))
    queues, missing_doccodes = build_qne_item_queues(Path(args.qne), invoice_by_doccode)
    ...
    ensure_columns(sheet, header_to_index, ["InvoiceID", "MatchedDocCode", "MatchedQNERow"])

    summary_sheet_name = "Invoice_ID_Match_Summary"
    unmatched_sheet_name = "Invoice_ID_Unmatched"
    ...
    for row_number in range(2, sheet.max_row + 1):
        ...
        if match_queue:
            ...
            sheet.cell(row=row_number, column=header_to_index["InvoiceID"], value=match["invoice_id"])
